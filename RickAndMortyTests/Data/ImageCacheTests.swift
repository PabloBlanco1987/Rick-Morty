...
struct ImageCacheTests {
    ...
    // La baja del interesado cancelado la da un Task aparte —onCancel es síncrono y
    // está fuera del actor—, así que hay que esperar a que llegue. Se cede el turno en
    // vez de dormir: no depende del reloj, así que no falla en una máquina lenta.
    private func waitUntilNothingIsInFlight(in cache: ImageCache) async {
        for _ in 0..<10_000 {
    ...
    // Un directorio propio por test, borrado al salir: si no, un test leería del disco
    // lo que ha dejado otro y los aciertos de caché dejarían de significar nada
