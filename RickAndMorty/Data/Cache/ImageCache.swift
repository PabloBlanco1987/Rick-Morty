...
actor ImageCache {
    ...
    // falte espacio.
    //
    // TODO: [Fase 06] Falta podar el directorio. Hoy crece sin límite y solo lo vacía
    // el sistema cuando necesita sitio. Se queda fuera de esta fase porque los 826
    // avatares de la API son unos 20 MB en el peor caso —cabe entero— y una poda LRU
    // hecha con prisa borra justo lo que se está usando. Entraría como un
    // `trim(to:)` llamado al pasar la app a segundo plano, ordenando los ficheros por
    // .contentAccessDateKey y borrando los más viejos hasta bajar del límite.
