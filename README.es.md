...
- **Código, identificadores, comentarios y textos de la interfaz, en inglés.** Los
  textos, además, nunca en el código: en `Localizable.xcstrings`, con su comentario.
  Esta documentación sí existe en los dos idiomas —`README.md` en inglés,
  `README.es.md` en español—, porque es la conversación sobre el *porqué* de cada
  decisión, y quien la lee no tiene por qué compartir el idioma del código.
...
- **Un `TODO` es un límite asumido, no deuda.** Lleva el marcador
  `[Out of scope · README §8]`, sus tres partes y su entrada en el §8; si algo no cabe
